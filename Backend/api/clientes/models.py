...
class Cliente(BaseModel):
    ...
    def get_restricciones_eliminacion(self):
        """
        Obtiene información detallada sobre las restricciones para eliminar el cliente.
        No se puede eliminar si tiene citas asociadas (en cualquier estado).
        """
        citas_count = 0
        try:
            from api.citas.models import Cita
            citas_count = Cita.objects.filter(cliente=self).count()
        except ImportError:
            pass
    ...
    def get_restricciones_cambio_estado(self):
        """
        Obtiene información detallada sobre las restricciones para cambiar el estado del cliente.
        No se puede cambiar estado si tiene citas asociadas (en cualquier estado).
        """
        citas_count = 0
        try:
            from api.citas.models import Cita
        ...
        return {
            ...
            'mensaje': f"Tiene {citas_count} cita(s) asociada(s) (en cualquier estado)"
        }
