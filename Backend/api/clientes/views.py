...
class ClienteViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        """
        cliente = self.get_object()
        restricciones = cliente.get_restricciones_eliminacion()
        
        if not restricciones['puede_eliminar']:
            return Response(
                {
                    'error': f'No se puede eliminar el cliente "{cliente.nombre}" porque {restricciones["mensaje"]}',
    ...
    def verificar_cambio_estado(self, request, pk=None):
        """
        Verificar si un cliente puede cambiar estado.
        Endpoint esperado por el frontend.
        """
        cliente = self.get_object()
        restricciones = cliente.get_restricciones_cambio_estado()
