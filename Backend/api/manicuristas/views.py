...
class ManicuristaViewSet(viewsets.ModelViewSet):
    ...
    def check_associations(self, request, pk=None):
        """
        Devuelve conteos de asociaciones que impiden eliminar:
        - Citas pendientes o en proceso
        - Novedades vigentes (no anuladas, desde hoy)
        - Liquidaciones pendientes (si el módulo existe)
        - Ventas/servicios asociados (si el módulo existe)
        """
        ...
        novedades_vigentes = Novedad.objects.filter(
            manicurista=manicurista,
            fecha__gte=hoy
        ).exclude(estado='anulada').count()
        liquidaciones_pendientes = (
            Liquidacion.objects.filter(manicurista=manicurista, estado='pendiente').count()
            if Liquidacion is not None else 0
        )
        ventas_asociadas = (
            VentaServicio.objects.filter(manicurista=manicurista).count()
            if VentaServicio is not None else 0
        )

        total_asociaciones = citas_activas + novedades_vigentes + liquidaciones_pendientes + ventas_asociadas
        can_delete = total_asociaciones == 0

        return Response({
            'puede_eliminar': can_delete,
            'totales': {
                'citas_activas': citas_activas,
                'novedades_vigentes': novedades_vigentes,
                'liquidaciones_pendientes': liquidaciones_pendientes,
                'ventas_asociadas': ventas_asociadas,
                'total_asociaciones': total_asociaciones,
            },
            'mensaje': None if can_delete else (
                "No se puede eliminar: "
                f"{citas_activas} cita(s) activa(s), "
                f"{novedades_vigentes} novedad(es) vigente(s), "
                f"{liquidaciones_pendientes} liquidación(ones) pendiente(s) y "
                f"{ventas_asociadas} venta(s) asociada(s)."
            )
        })
    ...
    def destroy(self, request, *args, **kwargs):
        """
        Eliminar manicurista solo si no tiene asociaciones bloqueantes.
        Si tiene asociaciones, no elimina y devuelve mensaje claro (estilo alerta, 400).
        """
        manicurista = self.get_object()
        ...
        if total_asociaciones > 0:
            return Response({
                'alerta': True,
                'mensaje': (
                    f"No se puede eliminar la manicurista '{manicurista.nombre}' porque tiene "
                    f"{citas_activas} cita(s) activa(s), "
                    f"{novedades_vigentes} novedad(es) vigente(s), "
                    f"{liquidaciones_pendientes} liquidación(ones) pendiente(s) y "
                    f"{ventas_asociadas} venta(s) asociada(s)."
        ...
        # Confirmación requerida cuando no hay asociaciones
        confirm = request.data.get('confirm') or request.query_params.get('confirm')
        if (confirm or '').strip().upper() != 'ELIMINAR':
            return Response({
                'alerta': True,
                'mensaje': (
                    f"Confirma la eliminación de la manicurista '{manicurista.nombre}' escribiendo 'ELIMINAR'."
                ),
                'requerido': 'ELIMINAR'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def cambiar_estado(self, request, pk=None):
        ...
        if manicurista.estado == 'activo':
            ...
            novedades_count = Novedad.objects.filter(
                manicurista=manicurista,
                fecha__gte=hoy
            ).exclude(estado='anulada').count()
            ...
            if Liquidacion is not None:
                liquidaciones_count = Liquidacion.objects.filter(
                    manicurista=manicurista,
                    estado='pendiente'
                ).count()

            if (citas_count + novedades_count + liquidaciones_count) > 0:
                # No cambiar estado; responder 200 con alerta detallada
                serializer = self.get_serializer(manicurista)
                return Response({
                    'alerta': True,
                    'mensaje': (
                        'No se puede inactivar: '
                        f"{citas_count} cita(s) pendiente/en proceso, "
                        f"{novedades_count} novedad(es) vigente(s) y "
                        f"{liquidaciones_count} liquidación(ones) pendiente(s)."
                    ).strip(),
                    'totales': {
                        'citas_activas': citas_count,
                        'novedades_vigentes': novedades_count,
                        'liquidaciones_pendientes': liquidaciones_count,
                    },
                    'manicurista': serializer.data
                }, status=200)
