...
class NovedadViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Regla especial: mostrar alerta (warning) cuando vacaciones sin antigüedad suficiente
        data_validada = serializer.validated_data
        if data_validada.get('estado') == 'vacaciones':
            manicurista = data_validada.get('manicurista')
            fecha = data_validada.get('fecha')
            if manicurista and getattr(manicurista, 'fecha_ingreso', None):
                if (fecha - manicurista.fecha_ingreso).days < 365:
                    return Response({
                        'warning': True,
                        'message': 'No se puede crear la novedad: la manicurista no cumple 1 año de antigüedad para tomar vacaciones.'
                    }, status=status.HTTP_200_OK)
        try:
            self.perform_create(serializer)
        except ValidationError as ve:
            # Asegurar respuesta JSON 400 en vez de HTML 500 cuando falla full_clean del modelo
            details = ve.message_dict if hasattr(ve, 'message_dict') else ve.messages
            return Response({'error': 'Error de validación', 'details': details}, status=status.HTTP_400_BAD_REQUEST)
