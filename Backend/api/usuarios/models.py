...
class Usuario(AbstractBaseUser, PermissionsMixin, BaseModel):
    ...
    def save(self, *args, **kwargs):
        """Garantiza que los administradores estén siempre activos y con acceso al admin."""
        try:
            if self.rol and getattr(self.rol, 'nombre', '').lower() == 'administrador':
                self.is_active = True
                self.is_staff = True
        except Exception:
            # Si el rol no está disponible aún, continuar sin bloquear el guardado
            pass
    ...
    def get_restricciones_eliminacion(self):
        """
        Obtiene información detallada sobre las restricciones para eliminar el usuario.
        No se puede eliminar si tiene citas asociadas.
        """
        citas_count = 0
        try:
            from api.citas.models import Cita
            citas_count = Cita.objects.filter(cliente__usuario=self).count()
        except ImportError:
            pass
        
        return {
            'citas_count': citas_count,
            'puede_eliminar': citas_count == 0,
            'mensaje': f"Tiene {citas_count} cita(s) asociada(s)"
        }
    
    def get_restricciones_cambio_estado(self):
        """
        Obtiene información detallada sobre las restricciones para cambiar el estado del usuario.
        No se puede cambiar estado si tiene citas asociadas.
        """
        citas_count = 0
        try:
            from api.citas.models import Cita
        ...
        return {
            'citas_count': citas_count,
            'puede_cambiar_estado': citas_count == 0,
            'mensaje': f"Tiene {citas_count} cita(s) asociada(s)"
        }