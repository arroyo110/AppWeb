...
class UsuarioViewSet(viewsets.ModelViewSet):
    ...
    # --- NUEVO: Activar administrador (marca is_active e is_staff) ---
    @action(detail=True, methods=['patch'], url_path='activar-admin')
    def activar_admin(self, request, pk=None):
        usuario = self.get_object()
        # Validar rol Administrador
        if not usuario.rol or usuario.rol.nombre.lower() != 'administrador':
            return Response(
                {"error": "Solo usuarios con rol 'Administrador' pueden ser activados como administradores"},
                status=status.HTTP_400_BAD_REQUEST
            )
        # Opcional: permitir elevar a superusuario si se especifica y tienes permisos
        elevar_super = str(request.data.get('is_superuser', 'false')).lower() in ['true', '1', 'yes']
        usuario.is_active = True
        usuario.is_staff = True
        if elevar_super:
            usuario.is_superuser = True
        usuario.save(update_fields=['is_active', 'is_staff'] + (['is_superuser'] if elevar_super else []))
        return Response({
            "mensaje": "Administrador activado correctamente",
            "usuario": UsuarioDetailSerializer(usuario, context={'request': request}).data
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            restricciones = usuario.get_restricciones_eliminacion()
            if not restricciones['puede_eliminar']:
                return Response(
                    {
                        'error': f'No se puede eliminar el usuario "{usuario.nombre}" porque {restricciones["mensaje"]}',
                        'detalle': restricciones['mensaje'],
    ...
    @action(detail=True, methods=['post'], url_path='resetear-contraseña')
    def resetear_contraseña(self, request, pk=None):
        """
        Resetear contraseña (solo admin) - genera nueva contraseña temporal
        """
        usuario = self.get_object()
        nueva_contraseña_temporal = usuario.generar_contraseña_temporal()
        usuario.save()
        
        # Enviar correo con nueva contraseña
        try:
            self.enviar_correo_reset_contraseña(usuario, nueva_contraseña_temporal)
        except Exception as e:
            print(f"Error enviando correo: {e}")
        
        return Response({
            'mensaje': f'Contraseña reseteada. Se envió un correo a {usuario.correo_electronico} con la nueva contraseña temporal y el enlace para cambiarla.'
        })
    
    def enviar_correo_reset_contraseña(self, usuario, nueva_contraseña):
        """
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <p>Hola <strong>{usuario.nombre}</strong>,</p>
                <p>Tu contraseña ha sido reseteada por el administrador. Aquí están tus nuevos datos de acceso:</p>
                
                <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #dc3545;">
                    <h3 style="margin-top: 0; color: #dc3545;">Nuevos datos de acceso:</h3>
                    <p><strong>Correo electrónico:</strong> {usuario.correo_electronico}</p>
                    <p><strong>Nueva contraseña temporal:</strong> <span style="color: #dc3545; font-weight: bold; font-size: 18px;">{nueva_contraseña}</span></p>
                </div>
                ...
                <div style="background-color: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ffc107;">
                    ...
                    <p>Esta es una contraseña temporal que debes cambiar en tu primer inicio de sesión por seguridad.</p>
                </div>
        ...
        Tu contraseña ha sido reseteada por el administrador.
        ...
        IMPORTANTE: Esta es una contraseña temporal que debes cambiar en tu primer inicio de sesión.
    ...
    def activar(self, request, pk=None):
        ...
        # Verificar restricciones de cambio de estado
        restricciones = usuario.get_restricciones_cambio_estado()
        if not restricciones['puede_cambiar_estado']:
            return Response(
                {
                    'error': f'No se puede cambiar el estado del usuario "{usuario.nombre}" porque {restricciones["mensaje"]}',
                    'detalle': restricciones['mensaje'],
                    'restricciones': restricciones
                },
                status=status.HTTP_400_BAD_REQUEST
    ...
    def verificar_eliminacion(self, request, pk=None):
        """
        ...
        return Response({
            'usuario_id': usuario.id,
            'usuario_nombre': usuario.nombre,
            'puede_eliminar': restricciones['puede_eliminar'],
            'citas_info': {
                'total': restricciones['citas_count']
            },
            'restricciones': restricciones
        })
    
    @action(detail=True, methods=['get'])
    def verificar_cambio_estado(self, request, pk=None):
        """
        ...
        return Response({
            'usuario_id': usuario.id,
            'usuario_nombre': usuario.nombre,
            'puede_cambiar_estado': restricciones['puede_cambiar_estado'],
            'citas_info': {
                'total': restricciones['citas_count']
            },
            'restricciones': restricciones
